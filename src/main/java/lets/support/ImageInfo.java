 ...
 * {@link #setDetermineImageNumber} with <code>true</code> as argument to identify animated GIFs ( {@link #getNumberOfImages()} will return
 * a value larger than <code>1</code>).</li>
 ...
 * <li><strong>2002-04-23</strong> Added support for the new properties physical resolution and comments for some formats. Released as
 * 1.2.</li>
 ...
 * skipped, leading to flaws in the retrieved information in some cases. Thanks to Bernard Bernstein for pointing that out. Released as
 * 1.4.</li>
 ...
 * Thanks to Martin Leidig for reporting this and sending in code. Released as 1.8.</li></li>
...
public class ImageInfo {
  ...
  * headers). Only one image can be stored in a file. It is determined whether the JPEG stream is progressive (see
  * {@link #isProgressive()}).
  */
 public static final int FORMAT_JPEG = 0;

 /**
  * Return value of {@link #getFormat()} for GIF streams. ImageInfo can extract comments from GIFs and count the number of images (GIFs with
  * more than one image are animations). It is determined whether the GIF stream is interlaced (see {@link #isProgressive()}).
  */
 public static final int FORMAT_GIF = 1;

 /**
  * Return value of {@link #getFormat()} for PNG streams. PNG only supports one image per file. Both physical resolution and comments can be
  * stored with PNG, but ImageInfo is currently not able to extract those. It is determined whether the PNG stream is interlaced (see
  * {@link #isProgressive()}).
  */
  ...
  * If true is returned, the file format was known and information on the file's content can be retrieved using the various getXyz methods.
  * 
 ...
 /**
  * If {@link #check()} was successful and {@link #setCollectComments(boolean)} was called with <code>true</code> as argument, returns the
  * number of comments retrieved from the input image stream / file. Any number &gt;= 0 and smaller than this number of comments is then a
 ...
 /**
  * Returns the physical height of this image in dots per inch (dpi). Assumes that {@link #check()} was successful. Returns <code>-1</code>
  * on failure.
  * 
