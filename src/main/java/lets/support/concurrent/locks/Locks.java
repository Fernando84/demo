...
/**
 * Utility methods to group-lock and group-unlock collections of locks, including roll back support to ensure that either all locks are
 * acquired or no locks are acquired.
 *
 * @author zhoupan
...
public class Locks {
 ...
 /**
  * Calls {@link java.util.concurrent.locks.Lock#lock()} on all locks provided by the given iterable, in the order provided by the iterable.
  * Automatically releases any locks acquired (in reverse order) by calling {@link java.util.concurrent.locks.Lock#unlock()} if an exception
  * is thrown, before re-throwing the exception.
  *
 ...
 /**
  * Calls {@link java.util.concurrent.locks.Lock#lockInterruptibly()} on all locks provided by the given iterable, in the order provided by
  * the iterable. Automatically releases any locks acquired (in reverse order) by calling {@link java.util.concurrent.locks.Lock#unlock()}
  * if the thread is interrupted while waiting for a lock or if an exception is thrown, before re-throwing the exception.
  *
  * @param locks The locks to acquire
  * @param <L>   Type of the lock
 ...
 /**
  * Calls {@link java.util.concurrent.locks.Lock#tryLock()} on all locks provided by the given iterable, in the order provided by the
  * iterable. Automatically releases any locks acquired (in reverse order) by calling {@link java.util.concurrent.locks.Lock#unlock()} if it
  * is not possible to obtain any lock, if the thread is interrupted while waiting for a lock, or if an exception is thrown, before
  ...
  * Calls {@link java.util.concurrent.locks.Lock#tryLock()} on all locks provided by the given iterable, in the order provided by the
  * iterable. Automatically releases any locks acquired (in reverse order) by calling {@link java.util.concurrent.locks.Lock#unlock()} if it
  * is not possible to obtain any lock within the remaining time within the timeout given, if the thread is interrupted while waiting for a
  * lock, or if an exception is thrown, before re-throwing the exception.
  *
 ...
 public static <L extends Lock> boolean tryLockAll(long time, TimeUnit unit, Iterable<L> locks) throws InterruptedException {
  ...
  try {
   ...
   for (L lock : locks) {
    long remainingNanos = !success ? limitNanos // No need to calculate remaining time in first iteration
 ...
 /**
  * Calls {@link java.util.concurrent.locks.Lock#unlock()} on all locks provided by the given iterable, in the <b>order provided</b> by the
  * iterable. <b>Note you may therefore wish to supply locks in reverse order.</b>
  *
