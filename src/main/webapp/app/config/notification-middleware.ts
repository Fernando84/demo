...
const addErrorAlert = (message: ToastMessage) => {
  toast.error(
    message.key ? (translate(message.key, message.data) ?? message.message) : message.message,
  );
};
...
const getFieldErrorsToasts = (fieldErrors: FieldErrorVM[]): ToastMessage[] =>
  fieldErrors.map(fieldError => {
    ...
    const fieldName = translate(`letsBankApp.${fieldError.objectName}.${convertedField}`);
    return {
      ...
      if (response.status === 401) {
        ...
        if (problem?.fieldErrors) {
          ...
          if (toastError) {
            ...
            toast.error(
              data?.detail ?? data?.message ?? data?.error ?? data?.title ?? 'Unknown error!',
            );
          }
        }
      }
