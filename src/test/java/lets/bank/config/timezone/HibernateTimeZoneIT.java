 ...
 *
 */
package lets.bank.config.timezone;
...
import lets.bank.repository.timezone.DateTimeWrapper;
import lets.bank.repository.timezone.DateTimeWrapperRepository;
...
class HibernateTimeZoneIT {
    ...
    private String generateSqlRequest(String fieldName, long id) {
        return format("SELECT %s FROM lets_date_time_wrapper where id=%d", fieldName, id);
    }
